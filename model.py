...
class NetL(torch.nn.Module):
    def __init__(self,
        ...
        self.convs = torch.nn.ModuleList()
        for i in range(config.layers):
            self.convs.append(ConvL(hidden_size=config.hidden, num_basis=num_basis, dropout=self.dropout_rate))
...
class ConvL(nn.Module):
    def __init__(self, hidden_size, num_basis, dropout=0.0):
    ...
    def forward(self, graph, x_feat, edge_attr, bases):
        with graph.local_scope():
            ...
            graph.edata['e'] = edge_attr
            graph.apply_edges(fn.u_add_e('x', 'e', 'pos_e'))
            graph.edata['v'] = self.pre_ffn(graph.edata['pos_e']) * self.filter_encoder(bases)
            graph.update_all(fn.copy_e('v', 'pre_aggr'), fn.sum('pre_aggr', 'aggr'))
            y = graph.ndata['aggr']
            x = x_feat + y
            y = self.ffn(x)
