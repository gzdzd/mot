...
def train_student_em(model, device, loader, optimizer, teacher_targets, labeled_indices, dist_lambda):
    ...
    for step, (bg, labels, indices) in enumerate(tqdm(loader, desc="Student Update")):
        ...
        if (~is_labeled).any():
            ...
            # Ensure indices are on CPU for indexing CPU tensor teacher_targets
            target_u = teacher_targets[indices.cpu()].to(device)
            target_u = target_u[mask_u]
            pred_u = pred1[mask_u]
...
def _load_smiles_aligned(data_dir, split_name):
    ...
    mol_graphs: 原始分子图数据列表
    """
    pickle_path = os.path.join('/home/zidong/mot/zinc/data/molecules', f"{split_name}.pickle")
    index_path = os.path.join('/home/zidong/mot/zinc/data/molecules', f"{split_name}.index")
    ...
    success_rate = sum(1 for s in smiles_list if s is not None) / len(smiles_list) * 100
    print(f"SMILES Generation Done. Success rate: {success_rate:.1f}%")
    return smiles_list, filtered_data
...
def main():
    ...
    if args.config is None:
        args.config = '/home/zidong/mot/zinc/ZINC.json'
    ...
    data_dir = '/home/zidong/mot/zinc/data/molecules'
    ...
    train_smiles, train_mol_graphs = _load_smiles_aligned(data_dir, 'train')
    train_labels = torch.tensor(raw_dataset.train.graph_labels).numpy()

    # 计算 SMILES 转换成功率
    success_rate = sum(1 for s in train_smiles if s is not None) / len(train_smiles) * 100
    print(f"SMILES Success Rate: {success_rate:.1f}%")

    # 根据 SMILES 成功率决定是否使用直接 Motif 提取
    use_direct_motif = success_rate < 80.0  # 成功率低于80%时使用直接 Motif 提取
    print(f"Will use {'direct motif extraction' if use_direct_motif else 'SMILES-based motif extraction'}")

    teacher = MotifGraphTeacher(train_smiles, train_labels, labeled_indices,
                                alpha=TEACHER_ALPHA)
