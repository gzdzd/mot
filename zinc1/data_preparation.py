...
class MoleculeDGL(torch.utils.data.Dataset):
    def __init__(self, data_dir, split, num_graphs, config):
    ...
    def get_smiles_list(self):
        """
        Helper to retrieve SMILES list for motif extraction.
        Requires that the underlying data source has 'smiles' or similar field.
        """
        # 这是一个假设的实现，具体取决于你的 pickle 文件结构
        # 必须返回一个长度为 num_graphs 的列表，包含 SMILES 字符串
        smiles_list = []
        # loading logic similar to _prepare...
        with open(self.data_dir + "/%s.pickle" % self.split, "rb") as f:
            data = pickle.load(f)
        # Filter by index
        with open(self.data_dir + "/%s.index" % self.split, "r") as f:
        ...
        for mol in data:
            # 假设数据字典里有 'smiles' 键，或者可以从 atom_type 重构
            if 'smiles' in mol:
                smiles_list.append(mol['smiles'])
            else:
                # Fallback: create dummy or try to reconstruct (difficult)
                # Warning: If no SMILES, MotifDiff cannot work.
                pass
        return smiles_list
...
class MoleculeDataset(torch.utils.data.Dataset):
    def __init__(self, name='Zinc', config=None):
        ...
        self.train = MoleculeDGL(data_dir, 'train', num_graphs=10000, config=self.config)
        self.val = MoleculeDGL(data_dir, 'val', num_graphs=1000, config=self.config)
        self.test = MoleculeDGL(data_dir, 'test', num_graphs=1000, config=self.config)
